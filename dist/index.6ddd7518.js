        ...
        if (hasStepsToValidate) {
            ...
            steps.forEach((type)=>{
                try {
                    ...
                    if (!allStepsAreValid) validator.setErrorFor($input, validator.validatorByType[type].message($input));
                } catch (error) {
                    throw new Error("Ops, aconteceu um erro na valida\xe7\xe3o do campo", error);
                }
            });
        }
        if (allStepsAreValid) validator.setSuccessFor($input);
        return allStepsAreValid;
    }
};

//# sourceMappingURL=index.6ddd7518.js.map
